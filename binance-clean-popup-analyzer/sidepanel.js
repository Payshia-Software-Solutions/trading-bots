...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-sync when switching tabs or window focus
...
async function checkActiveTab() {
  ...
  try {
    ...
    const isBinance = tab.url.includes('binance.com') && (tab.url.includes('/trade/') || tab.url.includes('/futures/'));
    if (isBinance) {
      showConnected();
      activeTabId = tab.id;
      
      // Parse active trading pair from tab URL
      const parsedPair = parsePairFromUrl(tab.url);
      if (parsedPair) {
    ...
    console.error('Error identifying tab:', error);
    showDisconnected();
  }
}

function parsePairFromUrl(url) {
  const match = url.match(/\/(trade|futures)\/([A-Z0-9_]+)/i);
  if (match && match[2]) {
    return match[2].replace('_', '').toUpperCase();
  }
  return null;
}
